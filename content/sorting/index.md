...
데이터가 거의 정렬되어 있는 상태라면 O(N)의 시간 복잡도를 갖는다.

### 📍 특징

보통의 경우 삽입 정렬은 비효율적이나, 데이터가 거의 정렬되어 있는 상태로 입력이 주어지는 경우에는 효율적이다.
데이터의 크기가 작고 거의 정렬되어 있는 상태라면 효율적이다.

## 🔗 **퀵 정렬 Quick Sort**

요약 : 기준 데이터(pivot)를 설정하고, 그 기준보다 큰 데이터와 작은 데이터의 위치를 바꾼다.
...
퀵정렬은 데이터가 **무작위로** 입력되는 경우 빠르게 동작할 확률이 높다.

**Worst : O(N^2)**

퀵정렬은 **데이터가 이미 정렬된 경우**에 매우 느리게 동작한다.

<!-- 퀵정렬의 시간 복잡도에 대한 증명은 복잡한 편이기 때문에 이번에는 넘어가도록 하자. 지금 단계에서는 구체적인 증명보다는 직관적인 이해에 초점을 맞추자. -->

### 📍 특징

데이터의 개수가 많을 수록 다른 정렬 알고리즘에 비해 압도적으로 빠르게 동작한다.
...
countingSort([7, 5, 9, 3, 0, 3, 1, 5, 6, 2, 4, 8]);
```
...
모든 데이터가 양의 정수인 상황에서 데이터의 개수를 N, 데이터 중 최댓값의 크기를 K

### 📍 특징

계수 정렬은 때에 따라 심각한 비효율성을 초래할 수 있다. 예를 들어 데이터가 0과 999,999 단 두 개만 존재한다고 가정해보자. 이럴 때도 배열의 크기가 100만개가 되도록 선언해야 한다. 따라서 항상 사용할 수 있는 정렬 알고리즘은 아니며, 동일한 값을 가지는 데이터가 여러개 등장할 때 적합하다. 예를 들어 성적의 경우 100점을 맞은 학생이 여러 명일 수 있기 때문에 계수 정렬이 효과적이다. 반면에 앞서 설명한 퀵 정렬은 일반적인 경우에서 평균적으로 빠르게 동작하기 때문에 데이터의 특성을 파악하기 어렵다면 퀵 정렬을 이용하는 것이 유리하다.
